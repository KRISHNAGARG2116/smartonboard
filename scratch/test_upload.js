...
async function run() {
  const scratchDir = path.resolve('/Users/krishnagarg/smartonboard-main/scratch');
  if (!fs.existsSync(scratchDir)) {
    fs.mkdirSync(scratchDir, { recursive: true });
  }

  // Create a dummy resume
  const dummyResumePath = path.join(scratchDir, 'dummy_resume.txt');
  ...
  await page.screenshot({ path: path.join(scratchDir, 'resumes_page_loaded.png') });
  ...
  await page.screenshot({ path: path.join(scratchDir, 'after_upload_attempt.png') });
