...
async function run() {
  ...
  const data = await page.evaluate(() => {
    ...
    // Let's find all headings, labels, button texts, or specific elements
    const elements = Array.from(document.querySelectorAll('*')).map(el => {
      // Find elements with text content or specific classes
    ...
    return {
      ...
      bodyTextSnippet: bodyText.substring(0, 2000),
      images,
      links,
      // Find unique class names
      classes: Array.from(new Set(elements.flatMap(e => e.className.split(' ')).filter(Boolean))).slice(0, 100)
    };
  });
