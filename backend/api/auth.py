...
# ---------------------------------------------------------------------------
# Candidate Registration & Authentication Endpoints
# ---------------------------------------------------------------------------
...
def candidate_send_email_otp(
    ...
    Uses the abstract OTP delivery provider and stores the token in
    the verification_tokens table.
    """
    email_lower = body.email.lower()
    ...
    # Generate OTP and store in verification_tokens
    ...
    # Send via abstract provider (Twilio/Mock)
...
def candidate_verify_email_otp(
    ...
    # Mark email as verified on the candidate profile
    with tenant_context(auth_mode="true"):
        profile = db.scalar(
...
def candidate_me(
    ...
    This endpoint is restricted to users with role=CANDIDATE.
    Internal scores and rankings are NEVER exposed.
    """
    with tenant_context(auth_mode="true"):
        profile = db.scalar(
...
# ---------------------------------------------------------------------------
# Candidate Resume Library Endpoints
# ---------------------------------------------------------------------------
