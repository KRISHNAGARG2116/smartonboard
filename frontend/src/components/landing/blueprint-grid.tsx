...
vec3 meshGradient(vec2 uv){vec2 p0=vec2(-.70,-.45),p1=vec2(.75,-.35),p2=vec2(-.65,.65),p3=vec2(.80,.55);vec3 c0=vec3(.20,.08,.12),c1=vec3(.26,.10,.15),c2=vec3(.16,.06,.10),c3=vec3(.32,.13,.19);float e=2.0;float w0=pow(1.0/(.2+distance(uv,p0)),e),w1=pow(1.0/(.2+distance(uv,p1)),e),w2=pow(1.0/(.2+distance(uv,p2)),e),w3=pow(1.0/(.2+distance(uv,p3)),e),ws=w0+w1+w2+w3;return(c0*w0+c1*w1+c2*w2+c3*w3)/ws;}
...
void mainImage(out vec4 outColor,in vec2 fragCoord){vec2 R=iResolution.xy;float t=iTime;vec2 uv=(fragCoord-.5*R)/max(R.y,1.);vec3 bg=mix(vec3(.04,.016,.024),vec3(.06,.024,.036),smoothstep(-.92,.55,-uv.y));bg=mix(bg,meshGradient(uv),MESH_AMT);bg*=clamp(pow(1.-VIGNETTE_AMT*length(uv),1.),0.,1.);vec2 uvAnim=uv+SCROLL_SPEED*t*normalize(vec2(1.,-.55));float thin=gridLineAA(uvAnim,GRID_SCALE,THIN_WIDTH),major=majorGridAA(uvAnim,GRID_SCALE,MAJOR_STEP,MAJOR_WIDTH);vec3 col=bg+vec3(.85,.70,.75)*thin*.25+vec3(.95,.85,.88)*major*.52;vec2 idx=floor(uvAnim*(GRID_SCALE/MAJOR_STEP)+.5);float selX=1.-step(.001,abs(fract(idx.x/ASCII_EVERY))),selY=1.-step(.001,abs(fract(idx.y/ASCII_EVERY)));if(ASCII_AMT>.001){vec2 cellF=fract(uv*ASCII_SCALE)-.5;float lvl=clamp(dot(col,vec3(.2126,.7152,.0722)),0.,1.);float glyph=asciiGlyph(cellF,lvl);float asciiMask=max(selX,selY)*major;vec3 asciiColor=mix(vec3(.90,.75,.78),meshGradient(uv),.25);col=mix(col,col+asciiColor*glyph*.30,ASCII_AMT*asciiMask);}col+=(vnoise(fragCoord*.6+vec2(t*12.,-t*9.))-.5)*NOISE_AMT;float luma=dot(col,vec3(.2126,.7152,.0722));col+=(bayer4(fragCoord)-.5)*mix(DITHER_DARK,DITHER_LIGHT,luma);outColor=vec4(tanh(col),1.);}void main(){mainImage(fragColor,gl_FragCoord.xy);}`
