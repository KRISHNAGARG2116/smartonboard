...
export function EnterpriseTrust() {
  return (
    <section id="enterprise" className="mx-auto max-w-6xl px-6 py-20 md:py-24">
      <Reveal className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-end">
        <div className="max-w-md">
          <SectionLabel>Enterprise trust</SectionLabel>
          <h2 className="mt-5 text-balance font-heading text-3xl leading-tight text-foreground md:text-4xl">
      ...
      <Reveal delay={0.1} className="mt-10 grid grid-cols-1 gap-px overflow-hidden rounded-[24px] border border-border bg-border sm:grid-cols-2 lg:grid-cols-4">
        {ITEMS.map((item) => (
          <div key={item.title} className="bg-card p-6">
            <span className="grid size-9 place-items-center rounded-xl bg-secondary text-foreground">
              <svg viewBox="0 0 24 24" className="size-4.5" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                <path d={item.icon} />
              </svg>
            </span>
